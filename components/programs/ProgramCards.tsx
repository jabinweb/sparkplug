  ...
  return (
    ...
    >
      <div className={order.includes("order-2") ? "order-1 lg:order-2" : ""}>
        {badge ? (
          <div
            className={`inline-flex items-center px-4 py-2 ${badgeColor} rounded-full text-sm font-medium mb-6`}
          >
            {badge}
          </div>
        ) : null}
        ...
        {idealFor.length > 0 && (
          <div className="mb-8">
            <p className="font-bold text-[var(--color-text-primary)] mb-4">
              Ideal for:
            </p>
            <ul className="grid grid-cols-2 gap-3">
              {idealFor.map((item, index) => (
                ...
                >
                  <svg
                    className="w-5 h-5 text-[var(--color-brand-primary)] mr-2 flex-shrink-0"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
      ...
      <div
        className={
          order.includes("order-2") ? "order-2 lg:order-1 relative group" : "relative group"
        }
      >
        <div
          className={`overflow-hidden rounded-3xl h-80 sm:h-96 relative shadow-2xl transition-all duration-500 group-hover:scale-[1.02] ${
            isImageUrl ? "bg-[var(--color-bg-tertiary)]" : gradientColors
          }`}
        >
          {isImageUrl ? (
            <div className="w-full h-full relative">
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-6 sm:p-8">
                <p className="text-white font-bold text-xl italic drop-shadow-lg">
                  &quot;{subtitle}&quot;
                </p>
              </div>
            </div>
            ...
            <div className="w-full h-full flex items-center justify-center p-8">
              <div className="text-center">
                <div
                  className={`w-28 h-28 ${iconBg} rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl`}
                >
                  <span className="text-6xl">{icon}</span>
                </div>
                <p className="text-[var(--color-text-secondary)] italic font-medium text-lg">
                  &quot;{subtitle}&quot;
                </p>
              </div>
            </div>
